...
const Board: React.FC<BoardProps> = ({ tiles, gridSize, onDeleteTile, onMoveTile, onSwapTiles, godMode }) => {
  ...
  const handleDragStart = (e: React.DragEvent, id: number) => {
    ...
    // Small delay to ensure the drag image is created correctly
    setTimeout(() => {
  ...
  const handleTileDragOver = (e: React.DragEvent, tId: number) => {
    if (!godMode) return;
    e.preventDefault();
    e.stopPropagation(); // Stop background cell from getting the event
    e.dataTransfer.dropEffect = 'move';
  ...
  // Empty cells for the background grid
  const cells = [];
  ...
  return (
    <div 
      className={`relative aspect-square w-full max-w-[500px] bg-slate-800/50 p-2 rounded-2xl border transition-all duration-300 shadow-2xl overflow-hidden backdrop-blur-sm ${
